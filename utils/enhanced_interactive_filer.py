...
class ProgressTrackerRegistry:
    ...
    def get_filing_status(self, filing_id: str) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            task = None
            with self.task_lock:
                if filing_id in self.tasks:
                    task = self.tasks[filing_id]

            # Get progress tracker if available
            tracker = self.tracker_registry.get_tracker(filing_id)

            # Get pending interaction if available
            interaction = None
            with self.interaction_lock:
                for interaction_id, req in self.pending_interactions.items():
                    if req.filing_id == filing_id:
                        interaction = req
                        break
            ...
            if task:
                ...
                if tracker:
                    status["progress"] = tracker.to_dict()

                # Add interaction information if available
                if interaction:
                    status["requires_interaction"] = True
                    status["interaction"] = interaction.to_dict()
                else:
                    status["requires_interaction"] = False

                return status
            elif tracker:
                # Task not found but tracker exists
                return {
                    "filing_id": filing_id,
                    "status": "unknown",
                ...
                # Neither task nor tracker found
    ...
    def get_all_filing_statuses(self) -> List[Dict[str, Any]]:
        """
        Get the status of all filings.
        ...
        try:
            ...
            # Get all tasks
            with self.task_lock:
                filing_ids = list(self.tasks.keys())

            # Get status for each filing
            for filing_id in filing_ids:
                status = self.get_filing_status(filing_id)
                if status:
            ...
            logger.error(f"Error getting all filing statuses: {str(e)}")
            return []
    ...
    def get_pending_interactions(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            with self.interaction_lock:
                for interaction_id, request in self.pending_interactions.items():
                    result.append(request.to_dict())

            return result

        except Exception as e:
            logger.error(f"Error getting pending interactions: {str(e)}")
            return []

    def get_browser_pool_status(self) -> Dict[str, Any]:
        """
        Get status of the browser pool.

        Returns:
            Browser pool status dictionary
        """
        return self.browser_pool.get_status()

    async def shutdown(self):
        """Shut down the interactive filer."""
        try:
            # Stop task workers
            self.stop_workers()
            ...
            logger.info("Enhanced interactive filer shut down")

        except Exception as e:
            logger.error(f"Error shutting down interactive filer: {str(e)}")

    async def process_filing(self, filing_id: str, application_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Process a filing directly (without scheduling).

        This method is provided for compatibility with the old interface,
        but scheduling is recommended for production use.
        ...
        try:
            # Create filing task
            task = FilingTask(filing_id, application_data)

            # Store the task
            with self.task_lock:
                self.tasks[filing_id] = task

            # Process the task directly
            result = await self._process_filing_task(task)

            # Update task
            with self.task_lock:
                if filing_id in self.tasks:
                    self.tasks[filing_id].result = result
                    self.tasks[filing_id].status = "completed" if result.get("status") == "success" else "failed"
                    self.tasks[filing_id].updated_at = datetime.now().isoformat()

            return result

        except Exception as e:
            logger.error(f"Error in direct filing process: {str(e)}")

            # Update task
            with self.task_lock:
                if filing_id in self.tasks:
                    self.tasks[filing_id].status = "failed"
                    self.tasks[filing_id].error = str(e)
                    self.tasks[filing_id].updated_at = datetime.now().isoformat()
